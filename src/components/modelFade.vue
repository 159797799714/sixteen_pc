		...
		<transition name="modal-fade">
			<div class="modal-container" v-show="visible" :style="{ width: width }">
				...
				<div class="modal-header">
					<div class="modal-title">{{$t("flash.a12")}}</div>
				...
				<div class="modal-footer" v-show="showOperation">
					<div class="modal-btn">
						<button class="cancel" ref="cancelBtn" @click="close" @mousedown="cancelMouseDown"
							@mouseup="cancelMouseUp">{{$t("flash.a11")}}</button>
						<button class="ok" @click="ok">{{$t("flash.a10")}}</button>
					</div>
				</div>
			</div>
		</transition>
